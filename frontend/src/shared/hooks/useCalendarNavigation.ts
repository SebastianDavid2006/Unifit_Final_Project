...
export interface CalendarNavigationDeps {
  monthNames: string[]
}

export interface CalendarNavigationReturn {
  getWeekStart: (d: Date) => Date
  getWeekEnd: (d: Date) => Date
  formatWeekRange: (d: Date) => string
  prevPeriod: (vistaCalendario: 'semana' | 'mes' | 'año', currentDate: Date) => Date
  nextPeriod: (vistaCalendario: 'semana' | 'mes' | 'año', currentDate: Date) => Date
}

export function useCalendarNavigation({ monthNames }: CalendarNavigationDeps): CalendarNavigationReturn {
