...
function Model() {
  ...
  const { scene } = useGLTF('/models/ui/objects/priority_bell/priority_bell.glb')

  useFrame((state) => {
    if (ref.current) {
      ref.current.rotation.z = Math.sin(state.clock.elapsedTime * 2.5) * 0.35
    }
  })
...
export function PriorityBellView() {
  return (
    <Canvas
