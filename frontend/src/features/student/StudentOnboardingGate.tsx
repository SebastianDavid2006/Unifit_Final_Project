...
import type { LoginSession } from '@/auth/pages/LoginPage'
...
interface OnboardingUser {
  ...
  email: string
  nombre: string
  onboarding: { cita: boolean; firma: boolean; huella: boolean }
...
export default function StudentOnboardingGate({ session, onLogout }: Props) {
  ...
  const [phase, setPhase] = useState<Phase>(() => {
    if (!user.onboarding.cita) return 'steps'
    if (!user.onboarding.firma || !user.onboarding.huella) return 'pending'
    return 'app'
  })
  ...
  const handleAgendaConfirm = (fecha: string, hora: string) => {
    setUser(prev => ({
      ...prev,
      onboarding: { ...prev.onboarding, cita: true },
      cita: { fecha, hora },
    }))
    setPhase('agendada')
  }
