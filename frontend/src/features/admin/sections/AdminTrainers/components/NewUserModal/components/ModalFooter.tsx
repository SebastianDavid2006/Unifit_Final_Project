...
export default function ModalFooter({ step, fingerprintStatus, onPrev, onCapture, onNext }: {
  ...
  const locked = step === 6 && fingerprintStatus !== 'captured'
  return (
    <div className={step === 5 ? 'relative z-10 flex-shrink-0 p-6 pt-4' : 'flex-shrink-0 p-6 pt-4'} style={{
      borderTop: step === 5 ? 'none' : '1px solid rgba(0,0,0,0.04)',
      background: step === 5 ? 'transparent' : 'rgba(255,255,255,0.8)',
    }}>
      <div className="flex items-center justify-between">
        ...
        <div className={step === 5 ? 'flex justify-center w-full' : 'flex-1 flex justify-end'}>
          <motion.button
            type="button"
            variants={{
            ...
            style={{
              background: locked ? 'rgba(0,0,0,0.15)' : step === 6 ? GREEN_GRAD : BLUE_GRAD,
