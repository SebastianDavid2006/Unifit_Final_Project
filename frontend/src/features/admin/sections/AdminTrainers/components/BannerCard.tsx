...
export default function BannerCard({ onOpenNewUser }: { onOpenNewUser: () => void }) {
  return (
    <motion.div className="relative rounded-3xl mb-8" style={{ background: 'linear-gradient(90deg, #FFFFFF 0%, #F8FBFF 40%, rgba(248,251,255,0) 100%)', boxShadow: '0 8px 30px rgba(0,0,0,0.03)' }}>
      ...
      <div
        style={{ position: 'absolute', left: 10, bottom: -40, width: 240, zIndex: 20, opacity: 0, animation: 'blur-fade 0.6s 0.3s ease forwards' }}
      >
        ...
        <img src={trainersImg} alt="Trainers" className="w-full h-auto drop-shadow-xl relative" style={{ transform: 'translateY(-15%)' }} />
      </div>
    </motion.div>
  )
}
