...
export default function BannerCard({ onOpenNewUser }: { onOpenNewUser: () => void }) {
  return (
    <motion.div className="relative rounded-3xl mb-16" style={{ background: 'linear-gradient(90deg, #FFFFFF 0%, #F8FBFF 40%, rgba(248,251,255,0) 100%)', boxShadow: '0 8px 30px rgba(0,0,0,0.03)' }}>
      ...
      >
        ...
        <img src={trainersImg} alt="Trainers" className="w-full h-full drop-shadow-xl relative" style={{ objectFit: 'cover', objectPosition: 'center bottom' }} />
      </div>
    </motion.div>
  )
}
