...
export default function TopbarSearch(props: Props) {
  const { value, onChange, focused, onFocusChange, placeholder, withFilters, filtersActive, onToggleFilters } = props
  return (
    <div className="flex-1 flex justify-center">
      <div className="flex items-center gap-2 max-w-md w-full">
        ...
        {withFilters && (
          <FiltersButton active={!!filtersActive} onClick={onToggleFilters} offset={focused} />
        )}
      </div>
    </div>
  )
