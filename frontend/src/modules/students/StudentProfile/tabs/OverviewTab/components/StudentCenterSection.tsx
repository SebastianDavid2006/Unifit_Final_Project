  ...
  return (
    <div className={`flex flex-col items-center relative ${className}`} style={{ paddingTop: 16, alignSelf: 'stretch', overflow: 'visible' }}>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.96 }}
        onClick={onShowInfo}
        className={`px-6 py-2.5 rounded-2xl text-sm font-bold text-white cursor-pointer transition-all duration-200 hover:shadow-xl ${isMobile ? 'mt-4 w-full max-w-xs' : 'absolute bottom-6 left-1/2 -translate-x-1/2 z-20'}`}
        style={{
          background: `
