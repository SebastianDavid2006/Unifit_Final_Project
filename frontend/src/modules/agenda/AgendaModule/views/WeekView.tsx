...
import { BLUE, BLUE_GRAD, GOLD_GRAD, dayLabels } from '../../AgendaData'
...
export function WeekView({ fullscreen, viewTitle, viewMode, onViewModeChange, onPrev, onNext, isExpanded, onToggleExpand, weekDates, todayStr, publishedDates, hoveredCol, hoveredHour, setHoveredCol, setHoveredHour, getApptsForDate, onSlotClick, hoverSlots = true }: WeekViewProps) {
  const dayHeader = (
  ...
  const slots = TIME_SLOTS_WEEK.map(t => (
    <div key={t} className={`grid grid-cols-8${hoverSlots ? ' transition-colors duration-200' : ''}`} style={{ borderBottom: '1px solid rgba(0,0,0,0.03)', background: hoverSlots && hoveredHour === t ? 'rgba(18,112,183,0.03)' : 'transparent' }}>
      ...
      {weekDates.map((dt, di) => {
        ...
        return (
          <div key={di} className="px-0.5 cursor-pointer transition-colors duration-200" style={{ minHeight: 30, background: hoveredCol === di ? (hoverSlots ? 'rgba(18,112,183,0.08)' : 'rgba(18,112,183,0.05)') : 'transparent' }}
            ...
            onClick={() => { if (appts.length === 0) onSlotClick(ds, t) }}>
            {appts.map(a => (
