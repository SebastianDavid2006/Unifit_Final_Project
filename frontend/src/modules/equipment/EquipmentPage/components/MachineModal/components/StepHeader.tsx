...
import { BLUE_GRAD, ORANGE_GRAD, RED } from '@/data/shared/constants'
...
export function StepHeader({ editingMachine, step, onConfirmClose, title }: StepHeaderProps) {
  return (
      ...
      <div className="flex items-center justify-center gap-1.5" style={{ marginTop: 12, marginBottom: 16 }}>
        {[1, 2, 3].map(s => (
          <motion.div
            ...
            animate={{
              width: s === step + 1 ? 16 : 6,
              background: s === step + 1 ? (editingMachine ? ORANGE_GRAD : BLUE_GRAD) : 'rgba(0,0,0,0.12)',
            }}
